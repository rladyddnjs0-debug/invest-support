...
*   **티커/종목명 검색**: 스크리닝 결과 내에서 티커 또는 종목명으로 즉시 필터링 (대소문자 무관, 부분 일치).
*   **관심종목 (⭐)**: 결과 테이블의 `⭐` 셀 클릭으로 관심종목 추가/제거. `data/watchlist.json`에 저장되어 앱 재시작 후에도 유지되며, 사이드바에서 관심종목만 필터링 가능.
...
*   **세이브티커 뉴스 연동**: 종목 상세 팝업에서 [saveticker.com](https://www.saveticker.com)의 해당 종목 뉴스 탭을 새 탭으로 바로 열어 확인.
